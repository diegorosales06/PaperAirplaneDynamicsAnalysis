...
from config import RHO, MASS, AREA, G, CSV_PATH
...
cosLift = [] # how much acceleration points along the lift direction
...
for normals, velocitynormals, accelerationNormals in zip(unitVectorNormal, unitVectorVelocity, unitVectorAcc):
    ...
    # acceleration normal: unit vector along acceleration
    cosLift.append(np.dot(normals, accelerationNormals)) # cosine of angle between lift and acceleration normal
    cosDrag.append(np.dot(-velocitynormals, accelerationNormals)) # cosine of angle between drag and acceleration normal
    cosWeightN.append(np.dot(normals, [0, -1])) # gravity on lift force
    cosWeightV.append(np.dot(-velocitynormals, [0, -1])) # gravity on direction of drag force
